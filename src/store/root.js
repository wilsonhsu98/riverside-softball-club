...
const actions = {
  ...
  lineLogin() {
    window.location = `${config.line.loginUrl}?from=${encodeURIComponent(
    ...
    auth
      ...
      .then(result => {
        ...
        if (user) {
          ...
          let providerData = result.user.providerData.find(
            item => item.providerId === result.credential.providerId
          );
          ...
          if (pendingCred) {
            ...
            promise = user.linkWithCredential(token).then(res => {
              ...
              providerData = res.providerData.find(
...
const mutations = {
  ...
  [types.SET_AUTH](state, auth) {
    const find = auth.find(
      item => item.team === window.localStorage.getItem("currentTeam")
    );
    if (find) {
