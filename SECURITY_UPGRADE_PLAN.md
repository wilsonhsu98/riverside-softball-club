...
- **2026-09-24**：上面那次「砍死重依賴」判斷錯誤，把 Netlify 的正式站 build 搞壞了（`build:server` 對 `firebase-functions` 噴 `Module not found`）。Netlify 的建置流程只在 repo 根目錄跑一次 `npm install`，從來不會進 `functions/` 再裝一次它自己的依賴，所以 `netlify-lambda build functions`（在根目錄執行、context 是 `functions/`）遇到任何非 `webpack.server.js` 裡 `externals`（目前只有 `firebase-admin`）的 import，都得從**根目錄** `node_modules` 才能解析成功——跟「有沒有被 `src/` 裡的程式碼直接 import」無關。本機當初判斷「沒用到」之所以沒抓到，是因為本機 `functions/node_modules` 早就裝過一份（給 `firebase deploy` 用），resolution 直接从那邊解掉，才會誤判成根目錄不需要。這次比對 `functions/package.json` 的完整依賴清單，並且**刻意清掉 `functions/node_modules` 模擬 Netlify 的乾淨環境**、實際跑 `build:server` 驗證過，才把下列套件加回根目錄 `package.json`（唯獨 `firebase-admin` 繼續留在 `externals`、不進根目錄——這個判斷本身是對的）：`cookie-parser`, `cors`, `crypto`, `express`, `firebase-functions`, `jsonwebtoken`, `multer`, `request`, `request-promise`, `serverless-http`, `simple-oauth2`, `slugid`, `url`。`http2`、`lottie-web` 當初砍掉是對的（`lottie-web` 只在 `loading.vue` 裡當 CDN `<script>` 字串用，`http2`/`crypto`/`url` 都是 Node 內建模組，npm 套件版本裝不裝都不影響 resolution）。**教訓**：這個 repo 的兩份 `package.json`（root 跟 `functions/`）看似獨立，但 Netlify 的 `build:server` 其實是用根目錄的 `node_modules` 去 bundle `functions/index.js`——以後要判斷「root 用不用得到 functions 的某個依賴」，不能只 grep `src/`，要嘛去讀 `webpack.server.js` 的 `externals` 清單，要嘛乾脆清空 `functions/node_modules` 後跑一次 `npm run build:server` 現場驗證，本機殘留的 `functions/node_modules` 會讓判斷完全失真。Dependabot 警告數字因此從 102 回升，這是必要的取捨，不是走回頭路——真正的修法仍是 Phase 1/2（把這些套件升級到有安全修補的版本），這兩個階段本來就會動到同一批套件。對應 commit：見本次 commit。
